...
- `src/teller_services/store.kotoba` — `Store` protocol + `MemStore`:
  registered accounts, committed records, an append-only audit
  ledger.
- `src/teller_services/advisor.kotoba` — `Advisor` protocol;
  ...
  confidence).
- `src/teller_services/governor.kotoba` — `TellerServicesGovernor/check`:
...
- `src/teller_services/actor.kotoba` — `build-graph`, `run-request!`,
