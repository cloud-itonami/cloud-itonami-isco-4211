(ns teller-services.advisor
  "TellerServicesAdvisor — proposes a teller-services operation
  (process a transaction, log a note, disburse cash above a
  threshold, override an identity verification) for a registered
  account. The advisor is swappable: `mock-advisor` (deterministic,
  ...
  account provenance or disbursement/identity-override risk;
  `teller-services.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :process|:log|:disburse-above-threshold|:override-identity-verification
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for account " (:account-id request))})
...
(def ^:private system-prompt
  "You are a teller-services advisor. Given a teller operation
