(ns teller-services.actor
  "TellerServicesActor — the ISCO-08 4211 independent teller-services
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one teller operation
  ...
  The unconditional invariant: the TellerServicesAdvisor can never
  directly commit a record or dispatch a robot action the
  TellerServicesGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [teller-services.advisor :as advisor]
            [teller-services.governor :as governor]
            [teller-services.store :as store]))

(defn build-graph
  "Build a compiled TellerServicesActor graph. `store` implements
  `teller-services.store/Store`. `advisor` implements
  `teller-services.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:account-id (:account-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
